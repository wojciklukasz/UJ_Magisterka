...
Mogą być to wartości pochodzące bezpośrednio z~sensorów, bazy danych lub z~publicznie dostępnych zbiorów.

Dane wejściowe zazwyczaj nie są wystarczającej jakości dlatego kolejny krok to ich oczyszczenie.
...
Do klasyfikacji stosuje się różnorakie podejścia, między innymi: support vector machine (SVM), random forest classifier (RFC), stochastic gradient descent (SGD), AdaBoost, hidden Markov models (HMM), linear discriminate analysis (LDA), k-nearest neighbor (k-NN), sztuczne sieci neuronowe~\cite{Varghese2015, Dzedzickis2020, Ko2018}.
...
Same wyrazy twarzy dla wielu emocji jest uniwersalny między członkami różnych kultur, płci i~niezależny od wieku.
...
Stanowią one bardzo ważną część komunikacji niewerbalnej, ruch dłoni jest drugim co do wielkości źródłem, mówiącym o~stanie emocjonalnym, po wyrazie twarzy~\cite{Noroozi2021}.
...
Rysunek~\ref{fig:body-representation} przedstawia dwa sposoby modelowania ludzkiego ciała w~komputerach~\cite{Noroozi2021}.
...
Zakres podstawowej częstotliwości głosu, około 50 — 500 Hz, jest o~wiele większe niż różnica między wypowiedzią neutralną i~w~stanie złości, która wynosi około 68 Hz~\cite{Calvo2015}.
...
Emocje wywołują również zmiany, których nie da się zaobserwować za pomocą ludzkich zmysłów.
...
Są to parametry, które można zmierzyć i~wnioskować na ich podstawie zmiany zachodzące w~organizmie.
...
Opisuje on zmiany w~przewodnictwie skóry spowodowane aktywnością gruczołów potowych.
...
Fotopletyzmografia (ang.~\textit{photoplethysmography (PPG)}) jest alternatywnym sposobem mierzenia aktywności.
...
Jest to typowe podejście wykorzystywane w~uczeniu nadzorowanym.
...
Przykładowo strach oraz złość są reprezentowane tak samo: wysokie pobudzenie i~niska przyjemność.
...
Ten trend był tym bardziej wzmacniany przez skupienie się na rozpoznawaniu emocji na podstawie twarzy.
...
Film może być analizowany na podstawie pojedynczych klatek, jednak sygnały biofizyczne lub mowa wymagają zazwyczaj większej ilości czasu, aby dać wartościowe dane.
